import { BrtLog, BrtLogConfig } from '@bruit/types';
...
export class ConsoleTool {
  private static logConfig: BrtLogConfig;
  private static logByLevel: { [level: string]: Array<BrtLog> } = {};

  static init(logConfig: BrtLogConfig) {
    if (
      !(
        !!(<any>window).cordova ||
  ...
  private static configure() {
    ...
    if (typeof (<any>JSON).retrocycle !== 'function') {
      (<any>JSON).retrocycle = function retrocycle($) {
        const pathTest = /^\$(?:\[(?:\d+|'(?:[^\\'\u0000-\u001f]|\\([\\'\/bfnrt]|u[0-9a-zA-Z]{4}))*')\])*$/;
        (function rez(value) {
      ...
      if (!(<any>console).overloaded.logArray) {
        ...
        (<any>console).logArray = () => {
          return Object.keys(ConsoleTool.logConfig.logCacheLength)
            .filter(key => ConsoleTool.logConfig.logCacheLength[key] > 0 && ConsoleTool.logByLevel[key])
            .map(key => ConsoleTool.logByLevel[key])
  ...
  private static handleLogMessage(type: BrtLogType, args: IArguments): Array<any> {
    if (type && args && args.length > 0) {
      ...
      if (ConsoleTool.logByLevel[currentLog.type].length > ConsoleTool.logConfig.logCacheLength[currentLog.type]) {
