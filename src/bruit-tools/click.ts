...
export class ClickTool {
  ...
  private static elementToTagString(
    ...
    let currentString = '';
    if (element.tagName) currentString += element.tagName.toLowerCase();
    if (element.id) currentString += ` id="${element.id}"`;
    if (element.className && typeof element.className === 'string' && element.className.length) currentString += ` class="${element.className.split(' ')[0]}..."`;
    if (element.getAttribute('type')) currentString += ` type="${element.getAttribute('type')}"`;
    if (element.getAttribute('name')) currentString += ` name="${element.getAttribute('name')}"`;
    if (element.getAttribute('href') && element.getAttribute('href') !== '#') currentString += ` href="${element.getAttribute('href')}"`;
    if (element.getAttribute('value')) currentString += ` value="${element.getAttribute('value')}"`;
